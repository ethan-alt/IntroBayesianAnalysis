...
data {
  ...
  real<lower=0> tau_slab_min;         // slab: tau ~ U(tau_slab_min, tau_slab_max)
  real<lower=tau_slab_min> tau_slab_max;
  real tau_spike_mean;                // mean parameter for spike prior on tau
  real tau_spike_sd;                  // sd parameter for spike prior on tau
}
parameters {
...
model {
  ...
  target += log_mix(
    spikeprob
    , normal_lpdf(tau_intercept | tau_spike_mean, tau_spike_sd)
    , uniform_lpdf(tau_intercept | 0, tau_slab_max)
  );
  target += log_mix(
    spikeprob
    , uniform_lpdf(tau_trteff | tau_slab_min, tau_slab_max)
    , normal_lpdf(tau_trteff | tau_spike_mean, tau_spike_sd)
  );
...
```{r}
pars <- c(
  'intercept', 'trteff', 'tau_intercept', 'tau_trteff', 'theta1', 'theta0'
