...
```{r, eval = FALSE}
library(cmdstanr)
model <- cmdstan_model('path/to/file/filename.stan')
```
...
called a `draws_df`.
```{r}
mcmc <- smpl$draws(format = 'draws_df')
mcmc %>% summarize_draws()
```

By default, the `summarize_draws()` method gives $90\%$ credible intervals. To
obtain a 95\% CI, we can do the following:
```{r}
mcmc %>% summarize_draws(quantile2, .args = list(probs = c(0.025, 0.975)))
```
...
mcmc2 <- smpl2$draws(format = 'draws_df')
mcmc2 %>% summarize_draws(
  'mean', 'sd', ~quantile(.x, probs = c(0.025, 0.975)), 'pgt0' = ~mean(.x > 0), 'pgt1' = ~mean(.x > 1)
)
```
